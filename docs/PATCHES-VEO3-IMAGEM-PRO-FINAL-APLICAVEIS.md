# PATCHES VEO 3.1 + IMAGEM PRO - FINAL APLICÁVEIS
...
### 3. lib/image-model-selector.ts (JÁ CORRETO - verificação)

```diff
# Arquivo já está correto com:
# - Nano -> gemini-2.5-flash-image
# - Pro -> gemini-3-pro-image-preview
# Nenhuma alteração necessária
```

### 4. lib/gemini-image-service-v2.ts (JÁ CORRETO - verificação)

```diff
# Arquivo já aceita primaryModel via config
# Nenhuma alteração necessária
```

### 5. app/api/creative/generate-image/route.ts (JÁ CORRETO - verificação)

```diff
# Arquivo já integra selectImageModel e passa imageModel ao brief
# Nenhuma alteração necessária
```

### 6. lib/feature-flags.ts (JÁ CORRETO - verificação)

```diff
# Arquivo já resolve request > tenant > env > default para todos os campos
# Nenhuma alteração necessária
```

### 7. env.example (CORRIGIDO)
...
### 8. app/criativos/page.tsx (JÁ CORRETO - verificação)

```diff
# Arquivo já possui:
# - Seletor Tipo (Imagem/Vídeo)
# - Seletor modelo de imagem (nano/pro)
# - Controles de vídeo (duration, ratio, variations)
# - Polling de status
# - Link de download
# Nenhuma alteração necessária
```
...
# git apply --check docs/PATCHES-VEO3-IMAGEM-PRO-FINAL-APLICAVEIS.md
